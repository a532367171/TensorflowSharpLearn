...
namespace MNISTTensorFlowSharp
{
    class Program
    {
        ...
        //private static void JDLZ()
        //{
        //    using (var graph = new TFGraph())
        //    {
        //        var session = new TFSession(graph);
        //        TFOutput x = graph.Placeholder(TFDataType.Float, new TFShape(-1, 784));
        //        TFOutput y_ = graph.Placeholder(TFDataType.Float, new TFShape(-1, 10));
        //        //模型的权重和偏移量
        //        Variable W = graph.Variable(graph.Zeros(new TFShape(784, 10)));
        //        TFOutput b = graph.Variable(graph.Zeros(new TFShape(10)));

        //        Variable y = graph.Softmax(graph.MatMul(x, W) + b);
        //    }
        //}
    }
}
